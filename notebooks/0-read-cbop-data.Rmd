...
cbop2018[prac_id != "" & data_waz_do != "" & data_waz_od != "" & kod_woj != "" &  oferta_eures == "false" & oferta_kraj == "false" ]
...
cbop2018 %>%
  ...
  mutate(sys_wynag = case_when(kod_syst_wynagr == "Czasowy ze stawką miesięczną" ~ "Monthly payment",
         ...
         kod_rodz_zatrud = ifelse(kod_rodz_zatrud %in% c("Umowa o pracę w zastępstwie", "Umowa na czas wyk. określonej pracy"),
                                  "other", kod_rodz_zatrud),
  ...
  filter(kod_rodz_zatrud != "other",
         !przyczyna_arch_dziedzinowa %in% c("anulowana", "odrzucona")) %>%
  select(kod_plac:data_waz_do,liczba_miejsc_og,kod_zawodu,zawod1,kod_syst_wynagr, sys_wynag, kod_rodz_zatrud,
         kod_zmianowosci, liczba_godz_tyg, kod_kategorii_oferty_pracy, przez_prac,
